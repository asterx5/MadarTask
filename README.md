...
This project uses MVVM. For more complex screens I would consider MVI — it enforces a single immutable state object and explicit user intents, which makes state transitions easier to trace and test. The key point is that switching between them is largely a **presentation-layer decision**: both patterns still delegate business logic to use cases, so moving from one to the other does not require touching the Domain or Data layers. It is more a style preference than an architectural commitment.
...
`UserDaoPagingTest` is an instrumented test suite that runs against an **in-memory Room database**. It uses `TestPager` from the `paging-testing` library to drive the `PagingSource` directly — no Compose, no ViewModel, no coroutine complexity.

Eight tests cover:

- First and second page sizes (pageSize = 20 enforced)
- Total item count across all pages
- Correct remainder on the last page
- Empty database handling
- NAME_ASC, NAME_DESC, and AGE_ASC sort correctness
- Two independent pagers reading the same dataset in opposite directions

One non-obvious fix that tests caught: `PagingConfig` defaults `initialLoadSize` to `pageSize × 3`, which means the first `refresh()` loads 60 items instead of 20. Setting `initialLoadSize = pageSize` explicitly in the test config corrects this.
